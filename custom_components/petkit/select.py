...
LIGHT_BRIGHTNESS_TO_PETKIT_NUMBERED = {
    v: k for (k, v) in LIGHT_BRIGHTNESS_NAMED.items()
}
...
IA_DETECTION_SENSITIVITY_TO_PETKIT = {
    v: k for (k, v) in IA_DETECTION_SENSITIVITY_NAMED.items()
}
...
async def async_setup_entry(
    ...
    for feeder_id, feeder_data in coordinator.data.feeders.items():
        ...
        if feeder_data.type in ["d4", "feedermini"]:
            selects.append(ManualFeed(coordinator, feeder_id))
        # D3 Feeder
        if feeder_data.type == "d3":
            selects.append(Sound(coordinator, feeder_id))
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        try:
            await self.coordinator.client.control_water_fountain(
                self.wf_data, ha_to_petkit
            )
        except BluetoothError:
            raise PetKitBluetoothError(
                f'Bluetooth connection to {self.wf_data.data["name"]} failed. Please try setting light brightness again.'
            )
        else:
            self.wf_data.data["settings"]["lampRingBrightness"] = (
                LIGHT_BRIGHTNESS_TO_PETKIT_NUMBERED.get(option)
            )
...
class WFMode(CoordinatorEntity, SelectEntity):
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        try:
            ...
            raise PetKitBluetoothError(
                f'Bluetooth connection to {self.wf_data.data["name"]} failed. Please try setting mode again.'
            )
        else:
...
class Sound(CoordinatorEntity, SelectEntity):
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        await self.coordinator.client.update_feeder_settings(
            self.feeder_data, FeederSetting.SELECTED_SOUND, ha_to_petkit
        )
...
class LBCleaningInterval(CoordinatorEntity, SelectEntity):
    ...
    def current_option(self) -> str:
        ...
        return CLEANING_INTERVAL_NAMED[
            self.lb_data.device_detail["settings"]["autoIntervalMin"]
        ]
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.CLEAN_INTERVAL, ha_to_petkit
        )
...
class LBLitterType(CoordinatorEntity, SelectEntity):
    ...
    async def async_select_option(self, option: str) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.SAND_TYPE, ha_to_petkit
        )
