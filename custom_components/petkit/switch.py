...
async def async_setup_entry(
    ...
    for lb_id, lb_data in coordinator.data.litter_boxes.items():
        ...
        if (lb_data.type == "t3") or ("k3Device" in lb_data.device_detail):
            switches.extend(
                (LBAutoOdor(coordinator, lb_id), LBPeriodicOdor(coordinator, lb_id))
            )
        # Pura MAX
        if lb_data.type == "t4":
            switches.extend(
                (LBContRotation(coordinator, lb_id), LBDeepCleaning(coordinator, lb_id))
            )
            # Pura MAX with Pura Air
            if "k3Device" in lb_data.device_detail:
                switches.append(LBDeepDeodor(coordinator, lb_id))
        # Purobot ULTRA
        if lb_data.type == "t6":
            switches.extend(
                (
...
class WFLight(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        try:
            await self.coordinator.client.control_water_fountain(
                self.wf_data, FountainCommand.LIGHT_ON
            )
        except BluetoothError:
            raise PetKitBluetoothError(
                f'Bluetooth connection to {self.wf_data.data["name"]} failed. Please try turning on the light again.'
            )
        else:
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        try:
            await self.coordinator.client.control_water_fountain(
                self.wf_data, FountainCommand.LIGHT_OFF
            )
        except BluetoothError:
            raise PetKitBluetoothError(
                f'Bluetooth connection to {self.wf_data.data["name"]} failed. Please try turning off the light again.'
            )
        else:
...
class WFPower(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        try:
            ...
            raise PetKitBluetoothError(
                f'Bluetooth connection to {self.wf_data.data["name"]} failed. Please try turning on the water fountain again.'
            )
        else:
            if self.wf_data.type == "w5":
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        try:
            await self.coordinator.client.control_water_fountain(
                self.wf_data, FountainCommand.PAUSE
            )
        except BluetoothError:
            raise PetKitBluetoothError(
                f'Bluetooth connection to {self.wf_data.data["name"]} failed. Please try turning off the water fountain again.'
            )
        else:
            if self.wf_data.type == "w5":
...
class WFDisturb(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        try:
            await self.coordinator.client.control_water_fountain(
                self.wf_data, FountainCommand.DO_NOT_DISTURB
            )
        except BluetoothError:
            raise PetKitBluetoothError(
                f'Bluetooth connection to {self.wf_data.data["name"]} failed. Please try turning on Do Not Disturb again.'
            )
        else:
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        try:
            await self.coordinator.client.control_water_fountain(
                self.wf_data, FountainCommand.DO_NOT_DISTURB_OFF
            )
        except BluetoothError:
            raise PetKitBluetoothError(
                f'Bluetooth connection to {self.wf_data.data["name"]} failed. Please try turning off Do Not Disturb again.'
            )
        else:
...
class IndicatorLight(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        if self.feeder_data.type == "feedermini":
            await self.coordinator.client.update_feeder_settings(
                self.feeder_data, FeederSetting.MINI_INDICATOR_LIGHT, 1
            )
        elif self.feeder_data.type == "feeder":
            await self.coordinator.client.update_feeder_settings(
                self.feeder_data, FeederSetting.FRESH_ELEMENT_INDICATOR_LIGHT, 1
            )
        else:
            await self.coordinator.client.update_feeder_settings(
                self.feeder_data, FeederSetting.INDICATOR_LIGHT, 1
            )
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        if self.feeder_data.type == "feedermini":
            await self.coordinator.client.update_feeder_settings(
                self.feeder_data, FeederSetting.MINI_INDICATOR_LIGHT, 0
            )
        elif self.feeder_data.type == "feeder":
            await self.coordinator.client.update_feeder_settings(
                self.feeder_data, FeederSetting.FRESH_ELEMENT_INDICATOR_LIGHT, 0
            )
        else:
            await self.coordinator.client.update_feeder_settings(
                self.feeder_data, FeederSetting.INDICATOR_LIGHT, 0
            )
...
class ChildLock(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        if self.feeder_data.type == "feedermini":
            await self.coordinator.client.update_feeder_settings(
                self.feeder_data, FeederSetting.MINI_CHILD_LOCK, 1
            )
        elif self.feeder_data.type == "feeder":
            await self.coordinator.client.update_feeder_settings(
                self.feeder_data, FeederSetting.FRESH_ELEMENT_CHILD_LOCK, 1
            )
        else:
            await self.coordinator.client.update_feeder_settings(
                self.feeder_data, FeederSetting.CHILD_LOCK, 1
            )
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        if self.feeder_data.type == "feedermini":
            await self.coordinator.client.update_feeder_settings(
                self.feeder_data, FeederSetting.MINI_CHILD_LOCK, 0
            )
        elif self.feeder_data.type == "feeder":
            await self.coordinator.client.update_feeder_settings(
                self.feeder_data, FeederSetting.FRESH_ELEMENT_CHILD_LOCK, 0
            )
        else:
            await self.coordinator.client.update_feeder_settings(
                self.feeder_data, FeederSetting.CHILD_LOCK, 0
            )
...
class ShortageAlarm(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_feeder_settings(
            self.feeder_data, FeederSetting.SHORTAGE_ALARM, 1
        )
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_feeder_settings(
            self.feeder_data, FeederSetting.SHORTAGE_ALARM, 0
        )
...
class VoiceDispense(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_feeder_settings(
            self.feeder_data, FeederSetting.SOUND_ENABLE, 1
        )
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_feeder_settings(
            self.feeder_data, FeederSetting.SOUND_ENABLE, 0
        )
...
class FeederDoNotDisturb(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_feeder_settings(
            self.feeder_data, FeederSetting.DO_NOT_DISTURB, 1
        )
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_feeder_settings(
            self.feeder_data, FeederSetting.DO_NOT_DISTURB, 0
        )
...
class SurplusControl(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_feeder_settings(
            self.feeder_data, FeederSetting.SURPLUS_CONTROL, 1
        )
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_feeder_settings(
            self.feeder_data, FeederSetting.SURPLUS_CONTROL, 0
        )
...
class SystemNotification(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_feeder_settings(
            self.feeder_data, FeederSetting.SYSTEM_SOUND, 1
        )
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_feeder_settings(
            self.feeder_data, FeederSetting.SYSTEM_SOUND, 0
        )
...
class LBAutoOdor(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.AUTO_ODOR, 1
        )
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.AUTO_ODOR, 0
        )
...
class LBAutoClean(CoordinatorEntity, SwitchEntity):
    ...
    def available(self) -> bool:
        ...
        if (self.lb_data.device_detail["state"]["pim"] != 0) and (
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.AUTO_CLEAN, 1
        )
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.AUTO_CLEAN, 0
        )
...
class LBAvoidRepeat(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.AVOID_REPEAT_CLEAN, 1
        )
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.AVOID_REPEAT_CLEAN, 0
        )
...
class LBDoNotDisturb(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.DO_NOT_DISTURB, 1
        )
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.DO_NOT_DISTURB, 0
        )
...
class LBPeriodicCleaning(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.PERIODIC_CLEAN, 1
        )
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.PERIODIC_CLEAN, 0
        )
...
class LBPeriodicOdor(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.PERIODIC_ODOR, 1
        )
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.PERIODIC_ODOR, 0
        )
...
class LBKittenMode(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.KITTEN_MODE, 1
        )
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.KITTEN_MODE, 0
        )
...
class LBDisplay(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.DISPLAY, 1
        )
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.DISPLAY, 0
        )
...
class LBChildLock(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.CHILD_LOCK, 1
        )
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.CHILD_LOCK, 0
        )
...
class LBLightWeight(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.DISABLE_LIGHT_WEIGHT, 1
        )
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.DISABLE_LIGHT_WEIGHT, 0
        )
...
class LBPower(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        await self.coordinator.client.control_litter_box(
            self.lb_data, LitterBoxCommand.POWER
        )
...
class LBContRotation(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.CONT_ROTATION, 1
        )
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.CONT_ROTATION, 0
        )
...
class LBDeepCleaning(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.DEEP_CLEAN, 1
        )
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.DEEP_CLEAN, 0
        )
...
class LBDeepDeodor(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.DEEP_REFRESH, 1
        )
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.DEEP_REFRESH, 0
        )
...
class PurifierLight(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_purifier_settings(
            self.purifier_data, PurifierSetting.LIGHT, 1
        )
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_purifier_settings(
            self.purifier_data, PurifierSetting.LIGHT, 0
        )
...
class PurifierTone(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_purifier_settings(
            self.purifier_data, PurifierSetting.SOUND, 1
        )
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_purifier_settings(
            self.purifier_data, PurifierSetting.SOUND, 0
        )
...
class LBCamera(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        """Turn camera on."""
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.CAMERA, 1
        )
    ...
    async def async_turn_off(self, **kwargs) -> None:
        """Turn camera off."""
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.CAMERA, 0
        )
...
class LBNightVision(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.NIGHT, 1
        )
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.NIGHT, 0
        )
...
class LBMicrophone(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        """Turn microphone on."""
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.MICROPHONE, 1
        )
    ...
    async def async_turn_off(self, **kwargs) -> None:
        """Turn microphone off."""
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.MICROPHONE, 0
        )
...
class LBSystemSound(CoordinatorEntity, SwitchEntity):
    ...
    async def async_turn_on(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.SYSTEM_SOUND, 1
        )
    ...
    async def async_turn_off(self, **kwargs) -> None:
        ...
        await self.coordinator.client.update_litter_box_settings(
            self.lb_data, LitterBoxSetting.SYSTEM_SOUND, 0
        )
